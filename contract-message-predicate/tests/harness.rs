...
async fn spend_predicate_with_script_constraint() {
    ...
    let mut script_bytecode = std::fs::read("../contract-message-script/out/debug/test-script.bin")
        .unwrap()
        .to_vec();
    ...
    let predicate_bytecode =
        std::fs::read("../contract-message-predicate/out/debug/test-predicate.bin").unwrap();
